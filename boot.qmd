...
> **Base bibliográfica:** estrutura didática baseada nos tópicos presentes em @chihara2022resampling \[Capítulos 5 e 7\], @voss2014statistical [Seção 5.2], @chernick2008bootstrap \[Capítulos 2, 3, 6, 7 e 9\], @chernick2011introduction [Capítulos 2, 3, 6 e 8] e @thulin2025modern [Seção 7.4].
...
@chihara2022resampling \[Seção 5.9, pp. 136--137\] observa que cerca de $1000$ reamostras podem ser suficientes para respostas aproximadas, enquanto recomenda números maiores, como $10\,000$ ou mais, quando se deseja boa precisão. @thulin2025modern [Seção 7.4, p. 266] recomenda, como regra prática moderna, $B=9\,999$ ou mais em análises em que o custo computacional permita. @voss2014statistical [pp. 205--206] também ressalta que alguns milhares de réplicas ainda podem produzir variação perceptível nos limites de intervalos.

::: {.callout-important title="Princípio prático"}
Não existe um valor universal de $B$ adequado para todos os objetivos.

- Para uma exploração didática ou estimativa preliminar de erro padrão, alguns milhares de réplicas podem ser suficientes.
- Para quantis de cauda usados em intervalos de confiança, é prudente usar um valor maior.
- O critério mais informativo é verificar a **estabilidade do resultado** quando $B$ aumenta.
...
Ausência de evidência estatística contra uma hipótese nula não é equivalente a evidência de ausência de efeito. Essa distinção é enfatizada por @chihara2022resampling [Capítulos 3 e 8].
:::
...
pode surgir como aproximação, via transformação inversa ou método delta, para a escala da correlação, mas não deve ser apresentada como “o erro padrão teórico exato” de $\hat\rho$ em geral. A transformação de Fisher é a referência mais adequada para explicar a aproximação normal.

*Base bibliográfica:* o exemplo da correlação e a necessidade de correção são discutidos em @chernick2008bootstrap [pp. 59--61].
:::
...
Se cada unidade produz um par $(X_i,Y_i)$, reamostrar $X$ e $Y$ independentemente destruiria a associação dentro do par. Há duas estratégias equivalentes em muitos problemas simples:

- reamostrar os pares $(X_i,Y_i)$;
- formar as diferenças $D_i=X_i-Y_i$ e reamostrar os $D_i$.
...
> **Base bibliográfica:** @chernick2008bootstrap \[Capítulo 4, pp. 78--96\] e @chernick2011introduction [Seções 2.4--2.6, pp. 56--67].
...
Além disso, para uma transformação estritamente monotônica $g$, os quantis transformam-se de forma coerente. Essa propriedade é frequentemente chamada de *transformation-respecting*.
:::
...
1.  **viés de localização**, representado por $z_0$;
2.  **mudança da variabilidade do estimador com o parâmetro**, representada pela constante de aceleração $a$.
...
::: {.callout-tip title="Interpretação"}
O $BC_a$ usa informação da distribuição bootstrap para corrigir viés e informação jackknife para ajustar a assimetria da precisão do estimador. Essa conexão é uma das razões pelas quais bootstrap e jackknife são pedagogicamente úteis no mesmo capítulo.
:::
...
@chernick2011introduction [Seção 3.5, pp. 85--88] também discute o intervalo **ABC** (*approximate bootstrap confidence*), concebido como aproximação computacionalmente eficiente de correções de ordem superior. Para um capítulo introdutório de Estatística Computacional, recomenda-se apresentar o $BC_a$ como método ajustado principal e deixar ABC, *tilting* e bootstrap iterado como extensões.
:::
...
|------------------|------------------|------------------|------------------|
| Normal | $\hat\theta \pm z\,\widehat{ep}$ | simples | depende de aproximação aproximadamente normal e simétrica |
...
| $BC_a$ | corrige viés e aceleração | geralmente mais adaptável que o percentil simples | cálculo mais complexo; ainda depende da adequação do bootstrap |
...
A inversão de intervalos de confiança pode fornecer testes correspondentes em muitos problemas contínuos, mas não deve ser aplicada mecanicamente a toda situação. @thulin2025modern [pp. 270--272] alerta para dificuldades importantes em certos problemas com parâmetros de distribuições discretas.
:::
...
> **Base bibliográfica:** @chernick2008bootstrap \[Seções 9.3 e 9.9\] e @chernick2011introduction [Seção 8.3].
...
*Base bibliográfica:* @chernick2008bootstrap \[Seção 6.2 e Capítulos 5 e 9\], @chernick2011introduction [Capítulos 5--6 e 8] e @chihara2022resampling [Seções 13.1--13.2].
:::
...
Em problemas regulares, os pseudovalores ajudam a interpretar a influência de cada observação sobre a estatística. A dispersão das réplicas leave-one-out também é exatamente a informação utilizada na estimativa da aceleração do intervalo $BC_a$.
...
O jackknife funciona particularmente bem para estatísticas suficientemente suaves, nas quais remover uma observação produz uma perturbação informativa e aproximadamente linear.

Para estatísticas não suaves, como alguns quantis e extremos, as réplicas podem assumir poucos valores distintos e a aproximação da variância pode ser deficiente. O bootstrap costuma oferecer maior flexibilidade nesses casos, embora também possa ter dificuldades com pequenas amostras e estatísticas extremas.

> **Base bibliográfica:** @chernick2008bootstrap \[Seção 6.1.1, pp. 115--116\] discute a relação entre jackknife e bootstrap para estimação de erro padrão e registra que o bootstrap pode apresentar desempenho superior em problemas nos quais a aproximação jackknife é limitada. @chernick2011introduction [Seção 1.2.1] apresenta a relação entre os dois métodos.
...
| Não linearidade | geralmente mais flexível | melhor para estatísticas suficientemente suaves |
...
::: {.callout-tip title="Exercício 1 — erro padrão bootstrap"}
...
::: {.callout-tip title="Exercício 2 — viés"}
...
::: {.callout-tip title="Exercício 3 — intervalos"}
...
::: {.callout-tip title="Exercício 4 — dois grupos independentes"}
...
::: {.callout-tip title="Exercício 5 — dados pareados"}
...
::: {.callout-tip title="Exercício 6 — escolha de B"}
...
::: {.callout-tip title="Exercício 7 — jackknife"}
Para
...
Compare os resultados com a função `jackknife()` apresentada no capítulo.
:::

::: {.callout-tip title="Exercício 8 — aceleração do BCa"}
...
Interprete o sinal e a magnitude obtidos. Em seguida, compare com o intervalo `boot.ci(..., type = "bca")`.
:::

::: {.callout-tip title="Exercício 9 — bootstrap versus permutação"}
...
::: {.callout-tip title="Exercício 10 — situação em que o bootstrap IID é inadequado"}
...
Compare as estimativas. Explique por que reamostrar pontos isoladamente destrói a dependência temporal. Pesquise, nos livros da pasta, como o *block bootstrap* procura corrigir esse problema.
