...
Antes da popularização dos computadores, gerar números aleatórios era tarefa manual ou mecânica: dados eram lançados, cartas eram embaralhadas, rodas giratórias eram utilizadas. Hoje, no entanto, confiamos no computador para criar sequências de números que **imitam o acaso**.  

Esses números são chamados de **pseudoaleatórios**: eles não são realmente aleatórios, pois são produzidos por regras determinísticas. Porém, se os parâmetros forem escolhidos corretamente, o resultado **“se parece”** muito com uma sequência de variáveis independentes e uniformes no intervalo $(0, 1)$.  

Um dos métodos mais antigos e influentes para isso é o **Gerador Congruencial Linear**, ou simplesmente **LCG (Linear Congruential Generator)**.  


### Gerador Congruencial Linear {.unnumbered}

O LCG produz uma sequência de inteiros $x_0, x_1, x_2, \dots$ segundo a regra:
...
- Para LCG multiplicativo, se $m$ for primo, existe a possibilidade de alcançar período máximo $m-1$.  
- Valores históricos:  
...
- Geradores modernos, como o **Mersenne Twister** (período \(2^{19937}-1\)) ou a família **PCG**, oferecem propriedades estatísticas superiores.
